...
function dk_speakup_emailpetition_shortcode( $attr ) {
	...
	if ( isset( $attr['id'] ) && is_numeric( $attr['id'] ) ) {
		...
		if ( $petition_exists ) {
			...
			//$petition->signatures = 7439;
			$progress_left  = $petition->goal - $petition->signatures;
			$progress_percent = $petition->signatures / $petition->goal * 100;
			...
			if ( ! $expired ) {
				...
				<div class="container">
					...
					<div class="dk-speakup-petition-wrap petition-wrap ' . $css_classes . '" id="dk-speakup-petition-' . $petition->id . '" ' . $width . '>
						<h2>' . stripslashes( esc_html( $petition->title ) ) . '</h2>
				...
				if ( $options['display_count'] == 1 ) {
					$petition_form .= '
						<h5>' . number_format( $petition->signatures ) . ' ' . _n( 'Signature', 'Signatures', $petition->signatures, 'dk_speakup' ) . '</h5>';
					if ( $progress_left > 0 ) {
						$petition_form .= '<span>' . number_format( $progress_left ) . ' more needed</span>';
					} else {
						$petition_form .= '<span>We made it! Keep on going!</span>';
					}
					$petition_form .= '<div class="progress"><div class="bar" style="width:' . $progress_percent . '%;"></div></div>';
					 				// . dk_speakup_SpeakUp::progress_bar( $petition->goal, $petition->signatures, $progress_width );
				}
				$petition_form .= '
						<p>' . stripslashes( esc_textarea( $petition->petition_message ) ) . '</p>
						<h4>Your Details</h4>
						<form class="dk-speakup-petition">
							...
							<fieldset class="dk-speakup-full">
								<input name="dk-speakup-first-name" id="dk-speakup-first-name-' . $petition->id . '" value="' . $userdata['firstname'] . '" type="text" placeholder="' . __( 'First Name', 'dk_speakup' ) . '" required/>
								<input name="dk-speakup-last-name" id="dk-speakup-last-name-' . $petition->id . '" value="' . $userdata['lastname'] . '" type="text" placeholder="' . __( 'Last Name', 'dk_speakup' ) . '" required/>
							</fieldset>
							<fieldset class="dk-speakup-full">
								<input name="dk-speakup-email" id="dk-speakup-email-' . $petition->id . '" value="' . $userdata['email'] . '" type="text" placeholder="' . __( 'Email', 'dk_speakup' ) . '" required/>';
				if ( $petition->requires_confirmation ) {
					$petition_form .= '
								<input name="dk-speakup-email-confirm" id="dk-speakup-email-confirm-' . $petition->id . '" value="" type="text" placeholder="' . __( 'Confirm Email', 'dk_speakup' ) . '" />';
				}
				if ( $petition->displays_custom_field == 1 ) {
					$petition_form .= '
								<input name="dk-speakup-custom-field" id="dk-speakup-custom-field-' . $petition->id . '" maxlength="400" type="text" placeholder="' . stripslashes( esc_html( $petition->custom_field_label ) ) . '"/>';
				}
				...
				if ( in_array( 'city', $petition->address_fields ) ) {
					$petition_form .= '
							<fieldset class="dk-speakup-half">
				...
				if ( $petition->is_editable == 1 ) {
					$petition_form .= '
							<fieldset class="dk-speakup-full dk-speakup-message-editable" id="dk-speakup-message-editable-' . $petition->id . '">
								<!-- <p class="dk-speakup-greeting">' . $petition->greeting . '</p> -->
								<textarea name="dk-speakup-message" class="dk-speakup-message-' . $petition->id . '" ' . $height . ' rows="8"></textarea>
								<!-- <p class="dk-speakup-caps">[' . __( 'signature', 'dk-speakup' ) . ']</p> -->
							</fieldset>';
				} else {
					$petition_form .= '
							<fieldset class="dk-speakup-full dk-speakup-message" ' . $height . ' id="dk-speakup-message-' . $petition->id . '">
								<p class="dk-speakup-greeting">' . $petition->greeting . '</p>
				...
				if ( $petition->displays_optin == 1 ) {
					...
					$petition_form .= '
							<fieldset class="dk-speakup-optin-wrap">
								<label for="dk-speakup-optin-' . $petition->id . '">
								<input type="checkbox" name="dk-speakup-optin" id="dk-speakup-optin-' . $petition->id . '"' . $optin_default . ' />
								' . stripslashes( esc_html( $petition->optin_label ) ) . '</label>
							</fieldset>';
				}
				$petition_form .= '
							<fieldset class="dk-speakup-submit-wrap">
								<div id="dk-speakup-ajaxloader-' . $petition->id . '" class="dk-speakup-ajaxloader" style="display:none;">&nbsp;</div>
								<a name="' . $petition->id . '" class="dk-speakup-submit"><button class="primary">' . stripslashes( esc_html( $options['button_text'] ) ) . '</button></a>
							</fieldset>
						</form>';
				$petition_form .= '
